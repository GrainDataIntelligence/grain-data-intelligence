...
METRIC_DEFINITIONS = {
    ...
    "oilseeds": [
        ...
        ("endingStocks", "Ending Stocks", "stock", ["Unutilised Closing stock"]),
        ("openingStock", "Opening Stock", "stock", ["Opening Stock"]),
        ("humanConsumptionLocal", "Human Consumption - Local Market", "single", ["Human consumption for Local Market"]),
        ("animalFeedLocal", "Animal Feed - Local Market", "single", ["Animal feed for Local Market"]),
        ("oilAndOilcakeLocal", "Oil and Oilcake - Local Market", "single", ["Oil and oilcake for Local Market"]),
        ("humanConsumptionCommercial", "Human Consumption - Commercial Market", "single", ["Human consumption for Commercial Market"]),
        ("animalFeedCommercial", "Animal Feed - Commercial Market", "single", ["Animal feed for Commercial Market"]),
        ("oilAndOilcakeCommercial", "Oil and Oilcake - Commercial Market", "single", ["Oil and oilcake for Commercial Market"]),
        ("seedForPlanting", "Seed for Planting", "single", ["Seed for planting purposes"]),
        ("releasedToEndConsumers", "Released to End Consumers", "single", ["Released to end-consumer(s)"]),
        ("withdrawnByProducers", "Withdrawn by Producers", "single", ["Withdrawn by producers"]),
        ("netDispatchesReceipts", "Net Dispatches / Receipts", "single", ["Net dispatches(+)/Receipts(-)"]),
        ("surplusDeficit", "Surplus / Deficit", "single", ["Surplus(-)/Deficit(+)"]),
        ("rsaExportHarbours", "RSA Export Harbours", "single", ["RSA Export Harbours"]),
        ("rsaExportBorderPosts", "RSA Export Border Posts", "single", ["RSA Export Border posts"]),
        ("productExportAfricanCountries", "Product Export African Countries", "single", ["Product Export African countries"]),
        ("productExportOtherCountries", "Product Export Other Countries", "single", ["Product Export Other countries"]),
    ],
    "maize": [
        ("producerDeliveries", "Producer Deliveries", "single", ["Deliveries directly from farms"]),
        ("imports", "Imports", "single", ["Imports destined for RSA"]),
...
def marketing_year_sort_key(marketing_year):
    text = str(marketing_year).strip()
    first_year = text.split("/")[0]
    if not first_year.isdigit():
        return None
    return (int(first_year), 1 if "/" in text else 0)


def is_before_marketing_year(marketing_year, minimum):
    current_key = marketing_year_sort_key(marketing_year)
    minimum_key = marketing_year_sort_key(minimum)
    if current_key is None or minimum_key is None:
        return False
...
def build_payload(config):
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        min_marketing_year = config.get("minMarketingYear")
        if min_marketing_year and is_before_marketing_year(marketing_year, min_marketing_year):
            continue
