...
function saveStoredLayouts(layouts) {
  localStorage.setItem(SAVED_ANALYSES_KEY, JSON.stringify(layouts));
}
...
function SavedAnalysisRow({ analysis, folders, onLoad, onRename, onUpdate, onDelete, onMove }) {
  const currentFolder = analysis.folderId || UNFILED_FOLDER_ID;

  return (
    <div className="group ml-3 flex items-center gap-1 rounded px-1 py-1 hover:bg-slate-100">
      <button
        type="button"
      ...
      <select
        value={currentFolder}
        onChange={(event) => onMove(analysis.id, event.target.value)}
        className="hidden h-6 w-14 rounded border border-slate-300 bg-white px-1 text-[10px] font-semibold text-slate-600 outline-none group-hover:block"
      ...
      <button
        type="button"
        onClick={() => onRename(analysis.id)}
        className="rounded px-1 text-[10px] font-bold text-slate-500 opacity-0 hover:bg-slate-200 group-hover:opacity-100"
        title="Edit name"
      >
        Edit
      </button>
      <button
        ...
        className="rounded px-1 text-[10px] font-bold text-blue-700 opacity-0 hover:bg-blue-50 group-hover:opacity-100"
        title="Update from current chart"
      >
        Upd
      </button>
      <button
        ...
        className="rounded px-1 text-[10px] font-bold text-red-600 opacity-0 hover:bg-red-50 group-hover:opacity-100"
        title="Delete"
      >
        Del
      </button>
...
export default function SeasonalCharts() {
  ...
  useEffect(() => {
    try {
      ...
      const folders = localStorage.getItem(SAVED_FOLDERS_KEY);
      if (saved) setSavedAnalyses(JSON.parse(saved));
      if (folders) {
        const parsedFolders = JSON.parse(folders);
        setSavedFolders(parsedFolders);
        setExpandedFolders(new Set([UNFILED_FOLDER_ID, ...parsedFolders.map((folder) => folder.id)]));
      }
    } catch (err) {
  ...
  const saveAnalysis = () => {
    ...
    const newAnalysis = {
      id: createId(),
      name: trimmed,
      folderId: saveFolderId === UNFILED_FOLDER_ID ? null : saveFolderId,
      timestamp: Date.now(),
      config: currentConfig,
    };
  ...
  const moveAnalysis = (analysisId, folderId) => {
    const nextFolderId = folderId === UNFILED_FOLDER_ID ? null : folderId;
    const updated = savedAnalyses.map((item) =>
      item.id === analysisId ? { ...item, folderId: nextFolderId } : item
    );
    setSavedAnalyses(updated);
    saveStoredLayouts(updated);
    if (nextFolderId) {
      setExpandedFolders((folders) => new Set([...folders, nextFolderId]));
    }
  };
  ...
  const addFolder = () => {
    ...
    if (!name?.trim()) return;
    const folder = { id: createId(), name: name.trim(), timestamp: Date.now() };
    const updated = [...savedFolders, folder];
    setSavedFolders(updated);
    saveStoredFolders(updated);
    setExpandedFolders((folders) => new Set([...folders, folder.id]));
    setSaveFolderId(folder.id);
  };

  const renameFolder = (folderId) => {
    const folder = savedFolders.find((item) => item.id === folderId);
    if (!folder) return;
    const name = prompt("Rename folder:", folder.name);
    if (!name?.trim()) return;
    const updated = savedFolders.map((item) => (item.id === folderId ? { ...item, name: name.trim() } : item));
    setSavedFolders(updated);
    saveStoredFolders(updated);
  };

  const deleteFolder = (folderId) => {
    if (!confirm("Delete this folder? Saved layouts inside it will move to Unfiled.")) return;
    const updatedFolders = savedFolders.filter((item) => item.id !== folderId);
    const updatedAnalyses = savedAnalyses.map((item) => (item.folderId === folderId ? { ...item, folderId: null } : item));
    setSavedFolders(updatedFolders);
    setSavedAnalyses(updatedAnalyses);
    saveStoredFolders(updatedFolders);
    saveStoredLayouts(updatedAnalyses);
    if (saveFolderId === folderId) setSaveFolderId(UNFILED_FOLDER_ID);
  };
  ...
  const savedGroups = useMemo(() => {
    const unfiled = savedAnalyses.filter((analysis) => !analysis.folderId);
    const folderGroups = savedFolders.map((folder) => ({
      ...folder,
      analyses: savedAnalyses.filter((analysis) => analysis.folderId === folder.id),
    }));
            ...
            <div className="flex h-[calc(100%-40px)] flex-col">
              <SidebarSection title="Saved Layouts">
                <div className="mb-3 rounded-md border border-slate-300 bg-white p-3 text-xs font-semibold leading-5 text-blue-700">
                  Save the current chart setup, then reopen or update it later from this list.
                </div>
                <div className="mb-2 flex items-center gap-2">
                  <select
                    ...
                    title="Folder for new saved layout"
                  >
                    <option value={UNFILED_FOLDER_ID}>Unfiled</option>
                  ...
                  <button
                    type="button"
                    onClick={addFolder}
                    className="h-8 rounded border border-slate-300 bg-white px-2 text-xs font-bold text-blue-700 hover:bg-blue-50"
                  >
                    Add Folder
                  </button>
                </div>
                ...
                <div className="max-h-[430px] overflow-y-auto rounded-md border border-slate-300 bg-white p-2">
                  {savedAnalyses.length === 0 ? (
                    ...
                    <div className="grid gap-1">
                      {savedGroups.unfiled.length > 0 && (
                        <div>
                          <button
                            type="button"
                            onClick={() => toggleFolder(UNFILED_FOLDER_ID)}
                            className="mb-1 flex w-full items-center gap-1 rounded px-1 py-1 text-left text-xs font-extrabold uppercase text-slate-600 hover:bg-slate-100"
                          >
                            <span className="w-3">{expandedFolders.has(UNFILED_FOLDER_ID) ? "v" : ">"}</span>
                            Unfiled
                          </button>
                          {expandedFolders.has(UNFILED_FOLDER_ID) &&
                            savedGroups.unfiled.map((analysis) => (
                              <SavedAnalysisRow
                                key={analysis.id}
                                analysis={analysis}
                      ...
                      {savedGroups.folders.map((folder) => (
                        <div key={folder.id}>
                          <div className="group flex items-center gap-1 rounded px-1 py-1 hover:bg-slate-100">
                            <button
                              type="button"
                              onClick={() => toggleFolder(folder.id)}
                              className="min-w-0 flex-1 truncate text-left text-xs font-extrabold text-slate-700"
                              title={folder.name}
                            >
                              <span className="mr-1 inline-block w-3">{expandedFolders.has(folder.id) ? "v" : ">"}</span>
                              {folder.name}
                            </button>
                            <button
                              type="button"
                          ...
                          {expandedFolders.has(folder.id) &&
                            (folder.analyses.length > 0 ? (
                              folder.analyses.map((analysis) => (
                                <SavedAnalysisRow
                                  key={analysis.id}
                                  analysis={analysis}
                                  folders={savedFolders}
                              ...
                              <p className="ml-4 px-1 py-1 text-[11px] font-semibold text-slate-400">Empty folder</p>
                            ))}
                        </div>
                      ))}
                    </div>
